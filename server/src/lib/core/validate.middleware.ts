import { BadRequestException, UnprocessableEntityException } from '@/constants/exceptions'
  ...
  return (req, res, next) => {
    ...
    if (result.error) {
      const errors = result.error.issues.map((issue) => ({
        ...
        path: issue.path,
        expected: (issue as any).expected, // todo: do type của zod chưa cập nhật đúng nên ép kiểu any tạm thời
      }))
